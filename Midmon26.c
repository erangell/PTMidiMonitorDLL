/****************************************/
/* MIDI MONITOR DLL version 2026.05.10  */
...
#define MAXTEMPOEVENTS 500	//2026-04-11 increased from 200 for songs like beethoven's 9th
...
void GetTransform(char * s_line)
{
    ...
    //2026-04-11 - removed to allow transfid to be a control change number                    
    //this requires the editor of the INI file to know what they're doing!
    //if (transfid != 0)
    //{   
    //    transfid = 1;
    //}
...
,"PTPA Midi Monitor DLL ver 2026.05.10", MB_OK);     
}
...
{                                           
    ...
    if (transform_at2vol == 1)
    {
        ...
        //2026-04-10: Use Expression controller instead of volume
        data1 = 11;
...
void ProcessChannelPressure (HDC myhdc, int sts, int data1, int data2)    
{
    ...
    if (transform_pr2vol >= 1)	//2026-04-11: changed from == to >=
    {
        sts = 0xB0 + chnl;  //make it a control change event
        data2 = data1;          //convert pressure value to vol ctrl value
        
        //data1 = 7;          //volume controller.  
        data1 = transform_pr2vol;          //2026-04-11: For EWI-USB wind controller
        //this allows user to put 11 in INI file to make breath control Expression  
    ...
    (HWND hwnd, WORD msg, int wParam, DWORD lParam)
{
    ...
    for (CurrTrack=1; CurrTrack<HighestTrack; CurrTrack++)
    {
        ...
        *cptr = (unsigned char *)lpvBuffer + 48;
        ...
        if ((PlayMuteStatus == 2) && (ForcedChannel != ExcludeDrumChannel))
        // track is playable and not looking at drum channel that user wants to exclude
        {
            Result = SendMessage(hPowerTracks
